			...
			if("Power")
				...
				if(ishuman(user))
					...
					if(!isshadowperson(human))
						to_chat(user, "<span class='warning'>Ваша плоть быстро мутирует!</span>")
						to_chat(user, "<b>Вы стали тенелюдом, представителем изменившейся расы гуманоидов, обитающих во тьме.</b>")
						to_chat(user, "<span class='warning'>Ваше тело бурно реагирует на свет.</span> <span class='notice'>При этом оно само по себе исцеляется в темноте.</span>")
						to_chat(user, "Помимо новых особенностей, вы не изменились ментально и сохранили свои прежние обязательства.")
