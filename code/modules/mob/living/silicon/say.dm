...
/mob/living/silicon/robot/handle_message_mode(message_mode, list/message_pieces, verb, used_radios)
	...
	if(message_mode)
		...
		if(!is_component_functioning("radio"))
			to_chat(src, "<span class='warning'>Радиомодуль не функционирует.</span>")
			return 0
...
/mob/living/silicon/ai/handle_message_mode(message_mode, list/message_pieces, verb, used_radios)
	...
	if(message_mode == "department")
		...
		if(aiRadio.disabledAi || aiRestorePowerRoutine || stat)
			to_chat(src, "<span class='danger'>Системная ошибка — приёмопередатчик отключён.</span>")
			return 0
		if(message_mode == "general")
			message_mode = null
...
/mob/living/silicon/ai/proc/holopad_talk(list/message_pieces, verb)
	...
	if(istype(T) && T.masters[src])
		...
		to_chat(src, "<i><span class='game say'>Ретранслированное голопадом сообщение, <span class='name'>[real_name]</span> [combine_message(message_pieces, verb, src)]</span></i>")
	else
		to_chat(src, "Голопады не подключены.")
		return
	return 1
...
/mob/living/silicon/ai/proc/holopad_emote(var/message) //This is called when the AI uses the 'me' verb while using a holopad.
	...
	if(istype(T) && T.masters[src])
		...
		to_chat(src, "<i><span class='game say'>Ретранслированное голопадом действие, <span class='name'>[real_name]</span> <span class='message'>[message]</span></span></i>")
