...
/datum/data/pda/app/main_menu/ui_act(action, list/params)
	...
	switch(action)
		if("UpdateInfo")
			...
			pda.name = "КПК — [pda.owner] ([pda.ownjob])"
		if("pai")
			if(pda.pai)
				if(pda.pai.loc != pda)
					pda.pai = null
...
/datum/data/pda/app/atmos_scanner/update_ui(mob/user as mob, list/data)
	...
	if(!isnull(T))
		...
		if(total_moles)
			...
			results = list(
				list("entry" = "Давление", "units" = "кПа", "val" = "[round(pressure,0.1)]", "bad_high" = 120, "poor_high" = 110, "poor_low" = 95, "bad_low" = 80),
				...
				list("entry" = "Оксид углерода", "units" = "%", "val" = "[round(co2_level*100,0.1)]", "bad_high" = 10, "poor_high" = 5, "poor_low" = 0, "bad_low" = 0),
				list("entry" = "Плазма", "units" = "%", "val" = "[round(plasma_level*100,0.01)]", "bad_high" = 0.5, "poor_high" = 0, "poor_low" = 0, "bad_low" = 0),
				list("entry" = "Прочее", "units" = "%", "val" = "[round(unknown_level, 0.01)]", "bad_high" = 1, "poor_high" = 0.5, "poor_low" = 0, "bad_low" = 0)
			)
